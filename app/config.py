...
class Settings(BaseSettings):
    ...
    service_description: str = Field(
        default="Sigma JSON-RPC 2.0 microservice over HTTP"
    )
    ...
    discovery_cache_ttl_seconds: int = Field(
        default=60, alias="SIGMA_DISCOVERY_CACHE_TTL_SECONDS"
    )
    ...
    allow_insecure_without_token: bool = Field(
        default=True, alias="SIGMA_ALLOW_INSECURE_WITHOUT_TOKEN"
    )
