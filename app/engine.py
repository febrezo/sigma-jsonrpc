...
class SigmaEngine:
    def __init__(self, settings: Settings) -> None:
        ...
        self._pipelines_cache: dict[str, _PipelineCache] = {}
    ...
    def list_plugins(self) -> list[PluginInfo]:
        ...
        if completed.returncode != 0:
            raise SigmaEngineException(
                _safe_err(completed.stderr, "sigma plugin list failed")
            )
    ...
    def list_backends(self, *, force_refresh: bool = False) -> list[str]:
        ...
        if (
            ...
            and self._backends_cache.expires_at > now
        ):
    ...
    def list_pipelines(self, target: str | None = None) -> list[str]:
        cache_key = (target or "__all__").strip().lower()
        now = time.time()
        cached = self._pipelines_cache.get(cache_key)
        if cached and cached.expires_at > now:
            entries = list(cached.items)
        else:
            entries = self._list_pipelines_uncached(target)
            self._pipelines_cache[cache_key] = _PipelineCache(
                expires_at=now + max(1, self.settings.discovery_cache_ttl_seconds),
                items=sorted(set(entries)),
            )
    ...
    def _list_pipelines_uncached(self, target: str | None = None) -> list[str]:
        entries: list[str]
        if target:
    ...
    def validate_rule(self, rule_text: str) -> ValidateResult:
        ...
        with _temp_rule_file(rule_text) as rule_path:
            validate_run = self._run_sigma(
                ["validate", rule_path], allow_validate_fallback=True
            )
            ...
            fallback_run = self._run_sigma(
                ["convert", "-t", "splunk", "--without-pipeline", rule_path]
            )
            if fallback_run.returncode == 0:
                return ValidateResult(valid=True, errors=[], warnings=[])

            error_text = _safe_err(
                fallback_run.stderr or validate_run.stderr, "rule validation failed"
            )
    ...
    def convert_rule(
        ...
        if completed.returncode != 0:
            raise SigmaEngineException(
                _safe_err(completed.stderr, "sigma convert failed")
            )
    ...
    def _list_targets_fast(self) -> list[str]:
        ...
        completed = self._run_sigma(
            ["list", "targets"], timeout=min(10, self.settings.sigma_command_timeout)
        )
        if completed.returncode != 0:
            return []
    ...
    def _run_sigma(
        ...
        *,
        ...
        try:
            ...
            logger.error(
                "sigma command timeout after %ss: %s", timeout_seconds, args[:2]
            )
            raise SigmaTimeoutException(
                f"sigma command timed out after {timeout_seconds}s"
            ) from exc
            ...
            if allow_validate_fallback:
                return subprocess.CompletedProcess(
                    command, returncode=2, stdout="", stderr=str(exc)
                )
    ...
    def _validate_identifier(self, value: str, *, field: str) -> str:
        ...
        if not cleaned or not _SAFE_IDENTIFIER_RE.match(cleaned):
            raise SigmaInputException(f"invalid {field}: must match [a-zA-Z0-9_-]+")
        return cleaned
...
def _parse_plugin_table(stdout: str) -> list[PluginInfo]:
    ...
    for idx, row in enumerate(rows):
        ...
        if "type" in normalized and any(
            item in normalized for item in ("id", "identifier", "name")
        ):
            header_idx = idx
    ...
    for row in rows[header_idx + 1 :]:
        ...
        try:
            plugin_type = (
                (row[header_map["type"]] if header_map.get("type") is not None else "")
                .strip()
                .lower()
            )
...
def _fallback_plugins_parse(lines: list[str]) -> list[PluginInfo]:
    ...
    for line in lines:
        parts = [
            part.strip() for part in re.split(r"\s{2,}|\t|\||│", line) if part.strip()
        ]
        if len(parts) < 2:
            continue
